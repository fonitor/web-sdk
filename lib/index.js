...
class HttpRequest {
  ...
  request(options, baseUrl) {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this.interceptors(instance, options.url);
...
class Queue {
  ...
  */
  constructor(options) {
    ...
    config = !util$1.isBlank(options) && util$1.isType().isPlainObject(options) ? Object.assign(config, options) : config; // 是否开启队列
    ...
    this.synRequestNum = !util$1.isBlank(config.synRequestNum) ? config.synRequestNum : 4; // 版本号
  ...
  onReady(fun) {
    ...
    if (util$1.isType().isfunction(fun)) {
      fun();
    }
  }
  /**
  ...
  delay(fun, e) {
    if (!util$1.isType().isfunction(fun)) return null;
...
class jsLog {
  ...
  recordJavaScriptError() {
    let siftAndMakeUpMessage = (origin_errorMsg, origin_url, origin_lineNumber, origin_columnNumber, origin_errorObj) => {
      ...
      let javaScriptErrorInfo = this.javaScriptErrorInfo(JS_ERROR, errorType + ": " + errorMsg, errorObj); // 进入队列
...
class rescourceLog {
  ...
  recordResourceError() {
    window.addEventListener('error', e => {
      ...
      if (!sourceUrl) return;
      if (sourceUrl) sourceUrl = util$3.b64EncodeUnicode(encodeURIComponent(sourceUrl));
...
const util$4 = Util.getInstance(),
      cache = [];
const queue$1 = Queue.getInstance('web');
class httpLog {
  constructor() {
  ...
  handleHttpResult(i, tempResponseText) {
    ...
    try {
      responseText = tempResponseText ? JSON.stringify(util$4.encryptObj(JSON.parse(tempResponseText))) : "";
    } catch (e) {
      responseText = "";
    }
    ...
    queue$1.pushToQueue(httpLogInfoStart);
    queue$1.pushToQueue(httpLogInfoEnd); // httpLogInfoEnd.handleLogInfo(error.HTTP_LOG, httpLogInfoEnd)
  ...
  httpLogInfo(uploadType, url, status, statusText, statusResult, responseText, currentTime, loadTime) {
    let obj = JSON.parse(JSON.stringify(util$4.getCommonProperty()));
    obj.uploadType = uploadType; // 上传类型

    obj.httpUrl = util$4.b64EncodeUnicode(encodeURIComponent(url)); // 请求地址
...
class Monitor {
  ...
  constructor(options) {
    ...
    util$6.WEB_BASE_URL = options.monitorBaseUrl || "";
